...
function getTagsFromMicroservice (){
  getToken_Config().then(
    function(response) {
      jsonToken = JSON.parse(response);
      var request = new XMLHttpRequest();
      request.setRequestHeader("Authorization", jsonToken.authToken);
...
function getToken_Config() {
  console.log("Making call to /secure/token_data");
  return new Promise(function(resolve, reject) {
    var request = new XMLHttpRequest();
    request.open('GET', '/secure/token_data');
    request.onload = function() {
      if (request.status == 200) {
        resolve(request.response);
      }
